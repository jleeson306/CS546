...
let createDailyStats = async function(name, date, calories, steps, water, etype, edur, calBurn) {	//what is this three new things, why add them??????????????????
	//if(arguments.length != 5 || name == undefined || date == undefined || calories == undefined || steps == undefined || water == undefined) {	//should be 5 if change back
	...
	// Leaving this here just in case my fix doesnt fix something?
	// let extraD = false;
	// if(etype != undefined || edur != undefined || calBurn != undefined) {
	// 	if(typeof etype !== 'string' || etype.trim() == "") {
	// 		throw("createDailyStats: Given Exercise Type must be a string.");
	// 	}
	// 	if(typeof edur !== 'number') {
	// 		throw("createDailyStats: Given Exercise Duration must be a number.");
	// 	}
	// 	if(typeof calBurn !== 'number') {
	// 		throw("createDailyStats: Given Calories Burned must be a number.");
	// 	}
	// 	extraD = true;
	// }
	...
	if(user == null) {
		...
		let stat = await usersCollection.findOne({userDisplayName: name, "dailyStats.date": date});
		if(stat == null) {
			let newDayStats = {};
			if(etype == undefined) {
				newDayStats = {
				...
				newDayStats = {
					...
					stepsTaken: steps,
					waterConsumed: water,
					dailyActivities: [{exerciseType: etype, exerciseDuration: edur, caloriesBurnt: calBurn}]
				};
			}
...
let getDailyStat = async function (username, date) {
	if(typeof username !== 'string' || username.trim() == "") {
		throw("getDailyStat: Given username must be a string.");
	}
	if(typeof date !== 'string' || date.trim() == "") {
		throw("getDailyStat: date must be a string with letters");
	}
	let beeboy = moment(date, 'YYYY-MM-DD', true).isValid();
	if(!beeboy) {
		throw("getDailyStat: date is invalid or in inncorect format");
	}
	...
	if(user == null) {
		throw("getDailyStat: No user with that name exists.");
	}
	...
	// console.log(stat);
	if (check) {
		throw("getDailyStat: No object for today exists.");
	}
	return stat;
}
let updateDailyStats = async function (username, date, calories, steps, water, etype, edur, calBurn) {
	const usersCollection = await users();
	const user = await usersCollection.findOne({userDisplayName: username});
	let dailyStatsList = [];
	...
	user.dailyStats.forEach(stat => {
		if (stat.date == date) {
				...
				if (etype || edur || calBurn) {
					stat.dailyActivities.push({exerciseType: etype, exerciseDuration: edur, caloriesBurnt: calBurn});
				}
